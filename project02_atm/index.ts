...
let pinAnswer = await inquirer.prompt([
  {
    message: chalk.greenBright("Enter your pin number"),
    type: "number",
    name: "pin",
  },
]);
  ...
  let chooseOption = await inquirer.prompt([
    {
      ...
      type: "list",
      choices: [
        chalk.whiteBright("Withdraw"),
        chalk.whiteBright("Fast payment"),
        chalk.whiteBright("Check balance"),
      ],
    },
  ]);
  if (chooseOption.operation === chalk.whiteBright("Withdraw")) {
    let getAmmount = await inquirer.prompt([
      {
        message: chalk.greenBright("Enter ammount you want"),
        name: "ammount",
        type: "number",
      },
    ]);
    if (getAmmount.ammount > myBalance) {
      ...
      console.log(
        chalk.yellow(
          `you Withdraw Rs ${getAmmount.ammount}\nyour remaining balance is ${myBalance}`
        )
      );
    }
    ...
    let selectAmmount = await inquirer.prompt([
      {
        name: "ammount",
        message: chalk.whiteBright("Please select ammount you want"),
        type: "list",
        choices: [
          chalk.whiteBright("1000"),
          chalk.whiteBright("2000"),
          chalk.whiteBright("3000"),
        ],
      },
    ]);
    if (selectAmmount.ammount === chalk.whiteBright("1000")) {
      let remaining: number = myBalance - 1000;
      console.log(
        chalk.yellow(
          `you Withdraw Rs ${3000}\nyour remaining balance is ${remaining}`
        )
      );
    } else if (selectAmmount.ammount === chalk.whiteBright("2000")) {
      let remaining: number = myBalance - 2000;
      console.log(
        chalk.yellow(
          `you Withdraw Rs ${3000}\nyour remaining balance is ${remaining}`
        )
      ...
      let remaining: number = myBalance - 3000;
      console.log(
        chalk.yellow(
          `you Withdraw Rs ${3000}\nyour remaining balance is ${remaining}`
        )

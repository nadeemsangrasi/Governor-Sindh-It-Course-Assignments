...
class MyTodo {
      ...
      let addList = await inquirer.prompt([
        {
          name: "todo",
          type: "input",
          message: chalk.bold("What do you want to add in your todo"),
        },
      ]);
      if (addList.todo !== "") {
        ...
        let addMore = await inquirer.prompt([
          {
            name: "more",
            type: "confirm",
            message: chalk.bold("Do you want to add more"),
          },
        ]);
        flage = addMore.more;
      } else {
  ...
  async updateTask() {
    ...
    if (todoList.length !== 0) {
      ...
      let toUpdate = await inquirer.prompt([
        {
          name: "index",
          type: "number",
          message: "Enter task number to update",
        },
      ]);
      if (
        toUpdate.index !== "" &&
        !Number.isNaN(toUpdate.index) &&
        toUpdate.index !== -1 &&
        toUpdate.index <= todoList.length
      ) {
        let newTask = await inquirer.prompt([
          { name: "task", message: "Enter new task name ", type: "input" },
        ]);
  ...
  async removeTask() {
    ...
    if (todoList.length !== 0) {
      ...
      if (
        toDelete.index !== "" &&
        !Number.isNaN(toDelete.index) &&
        toDelete.index !== -1 &&
        toDelete.index <= todoList.length
      ) {
